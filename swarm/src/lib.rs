...
*/    /*
    ...
    */
    ...
    event_receiver: mpsc::Receiver<SwarmEvent>,
...
// where
    ...
    async fn on_event(&mut self, event: SwarmEvent) -> Result<()> {
        match event {
    ...
    async fn on_command(&mut self, cmd: SwarmControlCmd<<TTrans::Output as StreamMuxer>::Substream>) -> Result<()> {
        ...
        match cmd {
            SwarmControlCmd::NewStream(peer_id, tx) => {
                // we got the peer_id, try dial_peer()
                if let Ok(stream) = self.dial_peer(peer_id).await {
            ...
            loop {
                ...
                match r {
                    Ok(muxer) => {
                        ...
                        tx.send(SwarmEvent::IncomingConnection { local_addr: Multiaddr::empty(), send_back_addr: Multiaddr::empty() }).await;
                    }
                    Err(err) => {
                        tx.send(SwarmEvent::ListenerClosed { addresses: vec![], reason: err }).await;
                    }
                }
            }
...
*/
    ...
    ///
    /// Returns an error if the address is not supported.
    pub async fn dial_addr(&mut self, addr: &Multiaddr) -> Result<()> {
        ...
        let mut conn = self.transport().clone().dial(addr.clone()).await?;
