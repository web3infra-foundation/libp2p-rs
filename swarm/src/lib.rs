...
#[derive(Debug)]
pub enum SwarmEvent<TStreamMuxer: StreamMuxer> {
    ...
    StreamOpened {
        // /// The sub stream.
        sub_stream: Substream<TStreamMuxer::Substream>,
    },
    ...
    TStreamMuxer::Substream: ReadEx + WriteEx + Send + Unpin + 'static,
{
    ...
    pub fn with_protocol(mut self, p: IProtocolHandler<TStreamMuxer::Substream>) -> Self {
    ...
    pub fn control(&self) -> Control<Substream<TStreamMuxer::Substream>> {
...
::Substream>>) -> Result<()> {
    ...
    pub fn network_info(&self) -> NetworkInfo {
        ...
        self.connections_by_id.iter().for_each(|(_id, c)| {
            connection_info.push(c.info())
        });
...
::Substream
    ...
    */
    ...
    fn handle_stream_opened(&mut self, sub_stream: Substream<TStreamMuxer::Substream>) -> Result<()> {
        log::trace!("handle_stream_opened: {:?}", sub_stream);
        let cid = sub_stream.cid();
        // add stream id to the connection substream list
        self.connections_by_id.get_mut(&cid).map(|c| c.add_stream(sub_stream));
